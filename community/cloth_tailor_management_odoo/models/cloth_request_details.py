...
class ClothRequestDetails(models.Model):
    ...
    def _default_stage_id(self):
        stage_id = self.env['cloth.request.stage'].search([], order='sequence',
                                                          limit=1).id
        return stage_id

    @api.model
    def _read_group_stage_ids(self, stages, domain):
        stages = self.env['cloth.request.stage'].search([])
    ...
    def _compute_is_readonly(self):
        repair_stage = self.env.ref('cloth_tailor_management_odoo.repair_stage_id').id
        start_stage = self.env.ref('cloth_tailor_management_odoo.start_stage_id').id
        for rec in self:
            if repair_stage == rec.stage_id.id or start_stage == rec.stage_id.id:
                rec.is_readonly = False
            else:
                rec.is_readonly = True

    @api.depends('total', 'deposit')
    def _compute_reminder(self):
        for rec in self:
            if rec.total and rec.deposit:
                rec.reminder = rec.total - rec.deposit
            else:
                rec.reminder = 0.0
    ...
    def action_sale_quotations_new(self):
        if not self.partner_id:
            raise ValidationError(_("Please select customer to create quotation."))
        else:
    ...
    def action_new_quotation(self):
        ...
        action = self.env['ir.actions.actions']._for_xml_id(
            'cloth_tailor_management_odoo.custom_sale_action_quotations_new')
        action['context'] = {
    ...
    def action_view_invoice(self):
        invoices = self.env['account.move'].sudo().search(
            [('custom_cloth_request_ids', 'in', self.id)])
    ...
    def action_view_measurement(self):
        ...
        action = self.env['ir.actions.actions']._for_xml_id(
            'cloth_tailor_management_odoo.action_cloth_measurement_details')
        if len(measurements) > 1:
            action['domain'] = [('id', 'in', measurements.ids)]
        elif len(measurements) == 1:
            form_view = [(self.env.ref(
                'cloth_tailor_management_odoo.cloth_measurement_details_form_view').id,
                          'form')]
            if 'views' in action:
                action['views'] = form_view + [(state, view) for state, view in
                                               action['views'] if
        ...
        for rec in self:
            if rec.cloth_type_id and not rec.measurement_ids:
                for line in rec.cloth_type_id.measurement_ids:
                    new_line_id = self.env[
                        'cloth.measurement.details.line'].create({
    ...
    def create(self, vals):
        for val in vals:
            if val.get('name', _('New')) == _('New'):
                val['name'] = self.env['ir.sequence'].next_by_code('cloth.request.details') or _('New')
